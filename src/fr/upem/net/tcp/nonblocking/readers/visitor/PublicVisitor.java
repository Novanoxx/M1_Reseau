...
public class PublicVisitor implements ReaderVisitor {
    @Override
    public int visit(Reader<Message> msgReader, ByteBuffer bufferIn) {
        Reader.ProcessStatus status = msgReader.process(bufferIn);
        switch (status) {
            case DONE:
                System.out.println("Public: " + msgReader.get().getLoginSrc() + " from " + msgReader.get().getServerSrc() + " : " + msgReader.get().getMsg());
                msgReader.reset();
                break;
            case REFILL:
                break;
